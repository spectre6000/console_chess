...
module ConsoleChess
  class Pawn < Space
    ...
    def unmoved
      ...
      @available_moves << en_passant_l if on_board?(en_passant_l) && empty?(up1) && capture?(en_passant_l)
      @available_moves << en_passant_r if on_board?(en_passant_r) && empty?(up1) && capture?(en_passant_r)
    end
